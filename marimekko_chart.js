/**
 * Visualização: Gráfico de Barras de Largura Variável (v2.0 Premium)
 *
 * ESTRUTURA DE DADOS:
...
looker.plugins.visualizations.add({
  ...
  create: function (element, config) {
    ...
    element.innerHTML = `
      <style>
        ...
        .premium-chart-legend { flex-shrink: 0; padding: 10px; overflow-y: auto; }
  ...
  updateAsync: function (data, element, config, queryResponse, details, doneRendering) {
    ...
    // --- REFINEMENT: Cálculo de Layout Responsivo com Espaçamento ---
    const totalSpacing = (data.length - 1) * config.bar_spacing;
    const margin = { top: 30, right: 20, bottom: 40, left: 60 };

    // Usa o tamanho real do elemento para o layout
    const availableWidth = element.clientWidth;
    const availableHeight = element.clientHeight;

    // Remove o espaço da legenda se ela estiver visível
    const legendWidth = (config.show_legend && config.color_by !== 'single') ? 150 : 0; // Estimar largura da legenda
    
    const chartWidth = availableWidth - margin.left - margin.right - legendWidth;
    const chartHeight = availableHeight - margin.top - margin.bottom;

    this._svg.selectAll('*').remove();
    this._svg.attr('width', chartWidth + margin.left + margin.right)
    ...
    // --- REFINEMENT: Escalas que consideram o espaçamento ---
    ...
    // --- REFINEMENT: Escala de Cores e Legenda Dinâmica ---
    let colorScale;
    switch(config.color_by) {
        case 'dimension':
            ...
            break;
        case 'measure_y':
            colorScale = d3.scaleSequential(d3.interpolateViridis) // Ou outra escala sequencial
                .domain([0, yMax]);
            break;
        case 'single':
        default:
    ...
    // --- REFINEMENT: Labels Inteligentes ---
  ...
  updateLegend: function(config, colorScale) {
    this._legend.html(''); // Limpa a legenda
    if (!config.show_legend || config.color_by === 'single') {
        return;
    }
